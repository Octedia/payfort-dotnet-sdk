...
{
    public class PurchaseResponseCommand : ResponseCommandWithNotification
    {
        private string _remember;
        private const string CommandKey = "command";

        [JsonPropertyName("command")]
        public override string Command => "PURCHASE";
        ...
        public override void BuildNotificationCommand(IDictionary<string, string> dictionaryObject)
        {
            ...
            var responseCommand = dictionaryObject.CreateObject<PurchaseResponseCommand>();

            if (responseCommand == null)
            {
                throw new InvalidNotification(
                    $"There was an issue when mapping notification request: [" +
                    $"{dictionaryObject.JoinElements(",")}] to PurchaseResponseCommand");
            }
        ...
        {
            var anonymized = new PurchaseResponseCommand()
            {
